...
const PastPlans = ({ userId }) => {
  const [plans, setPlans] = useState([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(null);
  const navigate = useNavigate(); // Initialize navigation

  // Function to fetch past plans
  const fetchPastPlans = async () => {
    try {
      ...
      console.log("Fetching past plans for user:", userId);
      const response = await fetch(`http://localhost:8000/api/data/get-PastPlans/${userId}`);

      if (!response.ok) {
        throw new Error("Failed to fetch past plans");
      }
    ...
    <div>
      ...
      <div className="cards-container">
        {plans.length > 0 ? (
          ...
          !loading && <p>No past plans available.</p>
        )}
      </div>
    </div>
  );
